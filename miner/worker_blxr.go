	...
	for {
		...
		case req := <-w.proposedCh:
			...
			if !ok || bestWorks == nil {
				...
				log.Info(log.MEVPrefix+"Received proposedBlock, this is the first proposed block for this block number and previous block hash", "blockNumber", req.args.blockNumber, "prevBlockHash", req.args.prevBlockHash.String(), "newProposedBlockReward", req.simulatedWork.reward, "newProposedBlockGasUsed", req.args.gasUsed, "mevRelay", req.args.mevRelay, "newProposedBlockTxCount", len(req.args.txs), "simulatedDuration", req.simDuration, "timestamp", time.Now().UTC().Format(timestampFormat))
